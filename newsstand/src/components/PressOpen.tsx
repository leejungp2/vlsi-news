...
import { getArticlesFor, HEADLINE_IMAGE_PLACEHOLDER } from "../data/articles";
...
/** "yyyy.MM.dd. HH:mm 편집" — 디자인 §6.10 head row */
...
export function PressOpen({
  ...
  const article = getArticlesFor(press.id);
  const editedAt = formatEditedAt(new Date(2026, 0, 14, 18, 53)); // 디자인 프레임 기준
  ...
  return (
    <div className={styles.open}>
      ...
      <div className={styles.body} aria-live="polite" aria-atomic="true">
        {/* 헤드 행 (28px tall, 디자인 §6.10): 워드마크 + 편집 시간 + 구독/해지 pill */}
        ...
        {/* 본문 (flex gap 24 margin-top 4, 디자인 §6.10):
         * 좌 340 (이미지+헤드라인) / 우 flex 1 (기사 6개 + footnote) */}
        <div className={styles.content}>
          <div className={styles.left}>
            <div className={styles.image} role="img" aria-label="기사 대표 이미지">
              <span className={styles.imageLabel}>
                {HEADLINE_IMAGE_PLACEHOLDER}
              </span>
            </div>
